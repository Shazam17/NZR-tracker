            ...
            subscriptions += TrelloRepository()
                ...
                .concatMap {
                    ...
                    if(it.body() != null){
                        ...
                        it.body()!!.forEach { task -> if(task.status.key.equals("open")) lsOpen.add(cardShort(task.id,task.summary))}
                        lists[0].cards.addAll(lsOpen)

                        var lsNeedInformation : MutableList<cardShort> = ArrayList()
                        it.body()!!.forEach { task -> if(task.status.key.equals("needInfo")) lsNeedInformation.add(cardShort(task.id,task.summary))}
                        lists[1].cards.addAll(lsNeedInformation)

                        var lsInWork : MutableList<cardShort> = ArrayList()
                        it.body()!!.forEach { task -> if(task.status.key.equals("inProgress")) lsInWork.add(cardShort(task.id,task.summary))}
                        lists[2].cards.addAll(lsInWork)

                        var lsClosed : MutableList<cardShort> = ArrayList()
                        it.body()!!.forEach { task -> if(task.status.key.equals("closed")) lsClosed.add(cardShort(task.id,task.summary))}
